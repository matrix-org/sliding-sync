...
func NewUserDeviceStream(userID, deviceID string, currPos sync3.Token) *UserDeviceStream {
...
func (s *UserDeviceStream) Broadcast(pos sync3.Token) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
// notified about
func (s *UserDeviceStreamListener) GetSyncPosition() sync3.Token {
...
// immediately.
func (s *UserDeviceStreamListener) GetNotifyChannel(sincePos sync3.Token) <-chan struct{} {
	s.userStream.lock.Lock()
	defer s.userStream.lock.Unlock()
